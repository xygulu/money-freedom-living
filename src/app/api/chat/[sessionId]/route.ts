...
export async function POST(request: NextRequest, ctx: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    if (isOpener && session.messageCount > 0) return jsonError('empty_message', 400); // opener 只属于空会话
...
async function respondToMessage(params: ReplyParams): Promise<Response> {
  ...
  async function* events() {
    ...
    const llmMessages = history.length > 0 ? history : [{ role: 'user' as const, content: '（对话开始）' }];
