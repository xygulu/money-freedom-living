...
export function buildAssessMessages(
  ...
  const system = [
    ...
    `  "actions": ["下一步可以做什么：2-3 个具体小步，对齐他所在阶段的练习方法（观察任务/微实验/允许清单/改写台词），小到今天就能开始；每条一句话，${actionLen}。第二人称，用「你」开头"],`,
    `  "nextHint": "下一阶段在远处长什么样：一句话，${nextLen}；第二人称；他已在门口就直说，还没到就诚实描述那段路",`,
    '}',
    '',
    '红线：',
    '- 证据必须来自素材：引用原话或具体的事，禁止编造',
    '- 所有给用户看的文字（summary/diagnosis/distance/actions/nextHint/每盏灯的 evidence）一律用第二人称「你」直接对他说话（英文用 you）——这份报告是念给他听的，不是向第三方汇报他；system 里用「他」指代他只是内部视角，不得带进任何输出字段',
